...
def update_flipper_ai(ball, left, right, frame, ai_state=None):
    ...
    def _side_ai(flipper, key):
        ...
        if st["mode"] == "flip":
            ...
            if st["flip_timer"] <= 0:
                st["mode"] = "idle"
                st["cooldown"] = FLIP_COOLDOWN_FRAMES
                return False
            return True  # keep raised — full snap + brief hold
        ...
        if on_blade and approaching:
            ...
            if ball.speed() > 1.6 and ball.vy > 0.4:
                st["mode"] = "flip"
                st["flip_timer"] = FLIP_HOLD_FRAMES
...
def update_upper_flipper_ai(ball, left, right, frame, ai_state=None):
    ...
    def _side_ai(flipper, key):
        ...
        if st["mode"] == "flip":
            ...
            if st["flip_timer"] <= 0:
                st["mode"] = "idle"
                st["cooldown"] = max(2, FLIP_COOLDOWN_FRAMES - 1)
                return False
            return True
        ...
        if will_hit and (ball.vy > -0.15 or ball.speed() > 0.9):
            st["mode"] = "flip"
            st["flip_timer"] = FLIP_HOLD_FRAMES + 1
            return True
...
def _right_outlane_top_xy():
    """
    Top of the right outlane: center of the wall↔guide channel, at the
    same y as the guide rail top (matches build_outlane_guides).
    """
    lane_l = _lane_left_x()
    flip_y = _playfield_bottom() - FLIPPER_BOTTOM_INSET
    y_top = flip_y - SLING_HEIGHT - SLING_UP_SHIFT - OUTLANE_GUIDE_ABOVE_SLING
    # Channel between plunger wall and outlane guide (~half OUTLANE_W inboard)
    x = float(lane_l - 0.5 * OUTLANE_W)
    x = min(x, float(lane_l) - BUMPER_R - 0.6)
    x = max(x, BUMPER_R + 1.5)
    return x, float(y_top)
...
def update_apron_drain_anim(anim, dt):
    ...
    Mid-game: to_score → hold → to_clock → done  → returns "relaunch"
    Final ball: to_score → hold (3s, Game Over glows) → screen_fade →
                returns "game_over"

    Returns None while running, or "relaunch" / "game_over" when finished.
    """
    ...
    if phase == "to_score":
        ...
        if anim["t"] >= 1.0:
            ...
            anim["t"] = 1.0
            hold_s = (
                GAME_OVER_SCORE_HOLD_SECONDS
                if anim.get("final_ball")
                else APRON_SCORE_HOLD_SECONDS
            )
            anim["hold_left"] = float(hold_s)
            anim["hold_total"] = float(hold_s)
    elif phase == "hold":
...
def _game_over_letter_pixels(text="GAME OVER", zoom=1):
    ...
    for ch in text:
        if ch == " ":
            specs.append((" ", None, max(2, 2 * zoom), max_h or 5 * zoom))
            continue
        sprite = None
        try:
            idx = ord(ch.upper()) - ord("A")
            if 0 <= idx < 26:
                sprite = LED.TrimSprite(copy.deepcopy(LED.AlphaSpriteList[idx]))
        except Exception:
            sprite = None
        if sprite is None:
            continue
        ...
        for count in range(sw * sh):
            ...
            for zv in range(zoom):
                for zh in range(zoom):
                    on.append((x * zoom + zh, y * zoom + zv))
        specs.append((ch, on, sw * zoom, sh * zoom))
        if sh * zoom > max_h:
            max_h = sh * zoom
    if not specs:
        return [], 0, 0
    total_w = sum(s[2] for s in specs) + gap * max(0, len(specs) - 1)
    total_h = max_h
    pixels = []
    x_cursor = 0
    for ch, on, lw, lh in specs:
        if ch == " " or on is None:
            x_cursor += lw + gap
            continue
        y_off = max(0, max_h - lh)
        for lx, ly in on:
            pixels.append((x_cursor + lx, y_off + ly))
        x_cursor += lw + gap
    return pixels, total_w, total_h


_GAME_OVER_CACHE = None
...
def draw_game_over_text(canvas, alpha=1.0, pulse=1.0):
    ...
    if alpha <= 0.02:
        return
    if _GAME_OVER_CACHE is None:
        _GAME_OVER_CACHE = _game_over_letter_pixels("GAME OVER", zoom=1)
    pixels, tw, th = _GAME_OVER_CACHE
    if not pixels or tw <= 0:
        return
    ox = max(0, (WIDTH - tw) // 2)
    ...
    if glow_k > 0.05:
        for lx, ly in pixels:
            for dx, dy in ((-1, 0), (1, 0), (0, -1), (0, 1)):
                sx, sy = ox + lx + dx, oy + ly + dy
                if 0 <= sx < WIDTH and 0 <= sy < HEIGHT:
                    set_px(
                        sx, sy,
                        int(gr * glow_k), int(gg * glow_k), int(gb * glow_k),
                    )
    for lx, ly in pixels:
        sx, sy = ox + lx, oy + ly
        if 0 <= sx < WIDTH and 0 <= sy < HEIGHT:
            set_px(
                sx, sy,
                int(cr * core_k), int(cg * core_k), int(cb * core_k),
            )


def _game_over_glow_alpha(anim):
    """0..1 glow progress during final-ball hold / fade."""
    if not anim:
        return 0.0
    phase = anim.get("phase")
    if phase == "screen_fade":
        return 1.0
    if phase != "hold" or not anim.get("final_ball"):
        return 0.0
    total = float(anim.get("hold_total") or GAME_OVER_SCORE_HOLD_SECONDS)
    left = float(anim.get("hold_left") or 0.0)
    elapsed = max(0.0, total - left)
    return _clamp(elapsed / max(0.05, GAME_OVER_GLOW_SECONDS), 0.0, 1.0)
...
def PlayPinball(Duration=10000, StopEvent=None):
    ...
    score_state = {"score": 0}
    # Balls remaining including the one currently in play / about to launch
    balls_remaining = int(BALLS_PER_GAME)
    exit_reason = None
    # Post-drain apron anim: clock scrolls out → score holds → clock / game over
    apron_anim = {
        "active": False,
        "phase": "done",
        "t": 0.0,
        ...
        while True:
            ...
            if ball.alive and ball.in_plunger:
                ...
                if (
                    ...
                    balls_remaining = max(0, balls_remaining - 1)
                    final_ball = balls_remaining <= 0
                    apron_anim["active"] = True
                    apron_anim["phase"] = "to_score"
                    apron_anim["t"] = 0.0
                    apron_anim["final_ball"] = final_ball
                    apron_anim["hold_left"] = (
                        GAME_OVER_SCORE_HOLD_SECONDS
                        if final_ball
                        else APRON_SCORE_HOLD_SECONDS
                    )
                    apron_anim["hold_total"] = apron_anim["hold_left"]
                    print(
                        f"[pinball] Ball drained — score={score_state['score']:05d}  "
                        f"balls_left={balls_remaining}  "
                        f"{'GAME OVER' if final_ball else 'apron clock→score'}"
                    )
                if apron_anim.get("active"):
                    apron_result = update_apron_drain_anim(
                        apron_anim, 1.0 / float(TARGET_FPS),
                    )
                    if apron_result == "relaunch":
                        apron_relaunch_ready = True
                    elif apron_result == "game_over":
                        exit_reason = "game_over"
                        print(
                            f"[pinball] Game over — final score="
                            f"{score_state['score']:05d}"
                        )
                        break
            ...
            if go_alpha > 0.02:
                ...
                if go_alpha < 1.0:
                    pulse = 0.9 + 0.1 * go_alpha
                draw_game_over_text(canvas, alpha=ease, pulse=pulse)
...
def _chain_next_pinball_table(Duration, StopEvent, from_table):
    ...
    if rem < 0.35:
        print(f"[pinball] Session nearly over ({rem:.2f} min) — no chain")
        return
    options = ["pinball", "pinball2"]
    others = [t for t in options if t != from_table]
...
def LaunchPinball(Duration=10000, ShowIntro=True, StopEvent=None):
    ...
    reason = PlayPinball(Duration=Duration, StopEvent=StopEvent)
