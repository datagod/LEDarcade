...
#   Version: 2.0                                                             --
#   Date:    July 9, 2026                                                    --
#   Reason:  Outbreak2 — fresh fork of Outbreak for experimentation          --
#------------------------------------------------------------------------------
...
def reset_strain_ids():
  global NextStrainId
  NextStrainId = 1


def obj_strain_id(obj):
  if obj is LED.EmptyObject or obj.name in ('EmptyObject', 'Wall', 'WallBreakable', 'OutOfBounds'):
    return 0
  return getattr(obj, 'strain_id', 0)


@njit
def find_closest_food_numba(cell_type_grid, source_h, source_v, radius, width, height):
  start_x = max(0, source_h - radius)
  stop_x = min(width, source_h + radius + 1)
  start_y = max(0, source_v - radius)
  stop_y = min(height, source_v + radius + 1)
  closest_x = -1
  closest_y = -1
  min_dist_sq = radius * radius + 1
  for y in range(start_y, stop_y):
    for x in range(start_x, stop_x):
      if cell_type_grid[y, x] == 2:
        dx = x - source_h
        dy = y - source_v
        dist_sq = dx * dx + dy * dy
...
@njit
def strain_at(strain_grid, h, v, width, height):
  if v < 0 or v >= height or h < 0 or h >= width:
    return 0
  return strain_grid[v, h]


@njit
def is_strain_nearby_numba(strain_grid, h, v, strain_id, width, height):
  if strain_id <= 0:
    return 0
  for dh in (-1, 0, 1):
    for dv in (-1, 0, 1):
      ...
      nh = h + dh
      nv = v + dv
      if 0 <= nh < width and 0 <= nv < height:
        if strain_grid[nv, nh] == strain_id:
          return 1
  return 0


@njit
def cell_type_at(cell_type_grid, h, v, width, height):
  if v < 0 or v >= height or h < 0 or h >= width:
    return -1
  return cell_type_grid[v, h]
...
VIRUS_START_COUNT         = 25      #initial and per-level virus population
TARGET_FPS                = 60      #stable frame pacing for LED output
TYPE_EMPTY                = 0
TYPE_WALL                 = 1
TYPE_FOOD                 = 2
TYPE_VIRUS                = 3
...
#Started out as an attempt to make cars follow shapes.  I was not happy with the results so I converted into a petri dish of viruses
  ...
  def __init__(self,name,width,height,Map,Playfield,CurrentRoomH,CurrentRoomV,DisplayH, DisplayV, mutationrate, replicationrate,mutationdeathrate,VirusStartSpeed):
    ...
    self.Viruses          = []
    self.RGBGrid          = None
    self.StrainGrid       = None
    self.CellTypeGrid     = None


  @staticmethod
  def _cell_rgb(obj):
    if obj is LED.EmptyObject or obj.name == 'EmptyObject':
      return (0, 0, 0)
  ...
  def _cell_meta(obj):
    if obj is LED.EmptyObject or obj.name == 'EmptyObject':
      return (0, 0, 0, 0, TYPE_EMPTY)
    if obj.name == 'WallBreakable':
      return (obj.r, obj.g, obj.b, 0, TYPE_FOOD)
    if obj.name == 'Wall':
      return (obj.r, obj.g, obj.b, 0, TYPE_WALL)
    return (obj.r, obj.g, obj.b, getattr(obj, 'strain_id', 0), TYPE_VIRUS)


  def rebuild_rgb_grid(self):
    self.RGBGrid = np.zeros((self.height, self.width, 3), dtype=np.uint8)
    self.StrainGrid = np.zeros((self.height, self.width), dtype=np.int32)
    self.CellTypeGrid = np.zeros((self.height, self.width), dtype=np.int8)
    for v in range(self.height):
      pf_row = self.Playfield[v]
      for h in range(self.width):
        r, g, b, strain_id, cell_type = self._cell_meta(pf_row[h])
        self.RGBGrid[v, h, 0] = r
        self.RGBGrid[v, h, 1] = g
        self.RGBGrid[v, h, 2] = b
        self.StrainGrid[v, h] = strain_id
        self.CellTypeGrid[v, h] = cell_type


  def sync_rgb_cell(self, v, h):
    r, g, b, strain_id, cell_type = self._cell_meta(self.Playfield[v][h])
    self.RGBGrid[v, h, 0] = r
    self.RGBGrid[v, h, 1] = g
    self.RGBGrid[v, h, 2] = b
    self.StrainGrid[v, h] = strain_id
  ...
  def set_playfield_empty(self, v, h):
    self.Playfield[v][h] = LED.EmptyObject
    self.RGBGrid[v, h, 0] = 0
    self.RGBGrid[v, h, 1] = 0
    self.RGBGrid[v, h, 2] = 0
    self.StrainGrid[v, h] = 0
    self.CellTypeGrid[v, h] = TYPE_EMPTY


  def set_virus_cell(self, v, h, virus):
    self.Playfield[v][h] = virus
    self.RGBGrid[v, h, 0] = virus.r
    self.RGBGrid[v, h, 1] = virus.g
    self.RGBGrid[v, h, 2] = virus.b
    self.StrainGrid[v, h] = virus.strain_id
    self.CellTypeGrid[v, h] = TYPE_VIRUS
  ...
  def AddRandomVirusesToPlayfield(self,VirusesToAdd=25):
    ...
    while (AddedCount <= VirusesToAdd):
      ...
      if (self.Playfield[v][h].name == 'EmptyObject' or
        ...
        virus = Virus(h,v,0,0,r,g,b,1,1, self.VirusStartSpeed,1,10,'',0,0,10,'West',0,self.mutationrate,0,self.replicationrate,self.mutationdeathrate)
        self.Playfield[v][h] = virus
        self.set_virus_cell(v, h, virus)
  ...
  def CopyTextMapToPlayfield(self,TextMap):
    ...
    for y in range (0,TextMap.height):
      for x in range (0,TextMap.width):
        ...
        if (dottype == "virus"):
          strain_id = int(mapchar) if mapchar.isdigit() else allocate_strain_id()
          self.Playfield[v][h] = Virus(h,v,0,0,r,g,b,1,1, self.VirusStartSpeed,1,10,str(strain_id),0,0,10,'West',0,self.mutationrate,0,self.replicationrate,self.mutationdeathrate,strain_id=strain_id)
  ...
  def DisplayWindow(self,h,v,ZoomFactor = 0):
    hat_w = LED.HatWidth
    hat_h = LED.HatHeight
    rgb_grid = self.RGBGrid
    set_pixel = LED.TheMatrix.SetPixel

    if ZoomFactor > 0:
      H_modifier = (1 / hat_w) * ZoomFactor * 2
      V_modifier = (1 / hat_h) * ZoomFactor
      NewHeight = round(hat_h * V_modifier)
      NewWidth  = round(hat_w * H_modifier)
      HIndentFactor = (hat_w / 2) - (NewWidth / 2)
      VIndentFactor = (hat_h / 2) - (NewHeight / 2)
      viewport = rgb_grid[v:v + hat_h, h:h + hat_w]
      for V in range(hat_h):
        row = viewport[V]
        for H in range(hat_w):
          pixel = row[H]
          set_pixel((H * H_modifier) + HIndentFactor, (V * V_modifier) + VIndentFactor, int(pixel[0]), int(pixel[1]), int(pixel[2]))
      return
    ...
    for V in range(hat_h):
      ...
      for H in range(hat_w):
        pixel = row[H]
        set_pixel(H, V, int(pixel[0]), int(pixel[1]), int(pixel[2]))
  ...
  def DisplayWindowZoom(self,h,v,Z1=8,Z2=1,ZoomSleep=0.05,max_steps=8):
    if Z1 <= Z2:
      zoom_levels = list(range(Z1, Z2))
    else:
      zoom_levels = list(reversed(range(Z2, Z1)))

    if len(zoom_levels) > max_steps:
      stride = max(1, len(zoom_levels) // max_steps)
      zoom_levels = zoom_levels[::stride][:max_steps]
  ...
  def FindClosestObject(self, SourceH, SourceV, Radius=10, ObjectType='WallBreakable'):
      if ObjectType == 'WallBreakable' and self.CellTypeGrid is not None:
        return find_closest_food_numba(
          self.CellTypeGrid, SourceH, SourceV, Radius, self.width, self.height)

      StartX = max(0, SourceH - Radius)
      StopX  = min(self.width, SourceH + Radius + 1)
      StartY = max(0, SourceV - Radius)
      StopY  = min(self.height, SourceV + Radius + 1)
      ClosestX = -1
      ClosestY = -1
...
class Virus(object):
  ...
  def __init__(self,h,v,dh,dv,r,g,b,direction,scandirection,speed,alive,lives,name,score,exploding,radarrange,destination,mutationtype,mutationrate,mutationfactor,replicationrate,mutationdeathrate,strain_id=0):
    ...
    self.alive           = 1
    self.lives           = 3
    self.strain_id       = strain_id if strain_id else allocate_strain_id()
    self.name            = name if name else str(self.strain_id)
    self.score           = 0
    self.exploding       = 0
    self.radarrange      = 20
    self.destination     = ""
  ...
  def clone(self):
    other = Virus(
      self.h, self.v, self.dh, self.dv,
      self.r, self.g, self.b,
      self.direction, self.scandirection, self.speed,
      self.alive, self.lives, self.name, self.score,
      self.exploding, self.radarrange, self.destination,
      self.mutationtype, self.mutationrate, self.mutationfactor,
      self.replicationrate, self.mutationdeathrate,
      strain_id=self.strain_id,
    )
    other.replications = self.replications
    other.mutations = self.mutations
    other.infectionchance = self.infectionchance
    other.chanceofdying = self.chanceofdying
    other.eating = self.eating
    other.clumping = self.clumping
    other.internalcounter = self.internalcounter
    return other
  ...
  def Mutate(self):
    ...
    if ((           fast_rng.randint(1,self.mutationdeathrate)          == 1)
      ...
      if (self.name != 'Wall' and self.name != 'WallBreakable'):
        self.strain_id      = allocate_strain_id()
        self.name           = str(self.strain_id)
      self.mutationtype   = mutationtype
...
def FlashAllViruses(Viruses,VirusCount,DinnerPlate,CameraH,CameraV):
  ...
  for x in range(VirusCount):
    ...
    highcolor = max(virus.r, virus.g, virus.b)
    while virus.r < 255 and virus.g < 255 and virus.b < 255:
      if virus.r == highcolor:
        virus.r = min(255, virus.r + increment)
      elif virus.g == highcolor:
        virus.g = min(255, virus.g + increment)
      elif virus.b == highcolor:
        virus.b = min(255, virus.b + increment)
      highcolor += increment
    DinnerPlate.sync_rgb_cell(virus.v, virus.h)
...
def IsThereAVirusNearby(ScanH, ScanV, direction, strain_id, DinnerPlate):
    return is_strain_nearby_numba(
      DinnerPlate.StrainGrid, ScanH, ScanV, strain_id, DinnerPlate.width, DinnerPlate.height)
...
def CountVirusesBehind(h,v,direction,strain_id,DinnerPlate):
  Playfield = DinnerPlate.Playfield
...
#         1 2 3    z = current location
  ...
  if strain_at(DinnerPlate.StrainGrid, ScanH, ScanV, DinnerPlate.width, DinnerPlate.height) == strain_id:
    count = count + 1
...
def SpreadInfection(Virus1,Virus2,direction):
  ...
  if (Virus2.name == "WallBreakable"):
    ...
    if(fast_rng.randint(1,InfectionChance) == 1):
      ...
      Virus2.strain_id = Virus1.strain_id
      Virus2.name = str(Virus1.strain_id)
      Virus2.r    = Virus1.r   
      Virus2.g    = Virus1.g   
      Virus2.b    = Virus1.b
...
def ReplicateVirus(Virus,DinnerPlate):
  ...
  if(Virus.replications <= MaxReplications):
    ...
    if (ItemList[5] == 'EmptyObject') or (ItemList[6] == 'EmptyObject'):
      ...
      DinnerPlate.set_virus_cell(ScanV, ScanH, VirusCopy)
      return VirusCopy; 
...
def MoveVirus(Virus, DinnerPlate):
  global VirusMoves
  ...
  if (WallInFront.name == "WallBreakable"):
    ...
    DinnerPlate.sync_rgb_cell(WallInFront.v, WallInFront.h)
    if (WallInFront.lives <= 0):
      DinnerPlate.set_playfield_empty(WallInFront.v, WallInFront.h)
  ...
  if (VirusInFront.name != 'EmptyObject' and VirusInFront.name != 'WallBreakable'):
    if (obj_strain_id(VirusInFront) != Virus.strain_id):
      SpreadInfection(Virus,VirusInFront,Virus.direction)

  #Check left diagonal Virus
  if (VirusLeftDiag.name != 'EmptyObject' and VirusLeftDiag.name != 'WallBreakable'):
    if (obj_strain_id(VirusLeftDiag) != Virus.strain_id):
      SpreadInfection(Virus,VirusLeftDiag,(LED.TurnLeft8Way(Virus.direction)))


  #Check right diagonal Virus
  if (VirusRightDiag.name != 'EmptyObject' and VirusRightDiag.name != 'WallBreakable'):
    if (obj_strain_id(VirusRightDiag) != Virus.strain_id):
      SpreadInfection(Virus,VirusRightDiag,(LED.TurnRight8Way(Virus.direction)))


  #Check rear Virus
  if (VirusInRear.name != 'EmptyObject' and VirusInRear.name != 'WallBreakable'):
    if (obj_strain_id(VirusInRear) != Virus.strain_id):
      SpreadInfection(Virus,VirusInRear,Virus.direction)


  #We follow other virus of the same strain
  if (obj_strain_id(VirusInFront) == Virus.strain_id and Virus.strain_id > 0):
    Virus.direction = VirusInFront.direction
  elif (obj_strain_id(VirusLeftDiag) == Virus.strain_id and Virus.strain_id > 0):
    Virus.direction = VirusLeftDiag.direction
  elif (obj_strain_id(VirusRightDiag) == Virus.strain_id and Virus.strain_id > 0):
    Virus.direction = VirusRightDiag.direction
  elif (obj_strain_id(VirusInRear) == Virus.strain_id and Virus.strain_id > 0):
  ...
  if (fast_rng.randint(0,Virus.mutationrate) == 1):
    ...
    if (Virus.alive == 0):
      ...
      DinnerPlate.set_playfield_empty(Virus.v, Virus.h)
  ...
  if (Virus.alive == 1 and Virus.eating == False):  
    ...
    if (TargetObject.name == 'EmptyObject'):
      ...
      if (Virus.clumping == True):
        if (IsThereAVirusNearby(ScanH, ScanV, Virus.direction, Virus.strain_id, DinnerPlate) == 1):
          Virus.h = ScanH
          Virus.v = ScanV
          DinnerPlate.set_virus_cell(ScanV, ScanH, Virus)
          DinnerPlate.set_playfield_empty(oldv, oldh)

        elif(CountVirusesBehind(Virus.h, Virus.v, Virus.direction, Virus.strain_id, DinnerPlate) == 0):
  ...
  DinnerPlate.sync_rgb_cell(Virus.v, Virus.h)
  for neighbor in (VirusInFront, VirusLeftDiag, VirusRightDiag, VirusInRear):
    if neighbor.name not in ('EmptyObject', 'OutOfBounds'):
      DinnerPlate.sync_rgb_cell(neighbor.v, neighbor.h)
...
def PlayOutbreak2(Duration,StopEvent):      
...
#  DinnerPlate = CreateDinnerPlate(LevelCount)
  reset_strain_ids()
  DinnerPlate = CreateDinnerPlate(width=80, height=45, virus_count=VIRUS_START_COUNT)
  ...
  while (finished == "N" and VirusMoves < MaxVirusMoves and LevelsPlayed <= MaxLevelsPlayed and VirusCount > 0):
    if StopEvent and StopEvent.is_set():
      print("\n" + "="*40)
      print("[Outbreak2] StopEvent received")
    ...
    NameCount = len({virus.strain_id for virus in DinnerPlate.Viruses})
    ...
    while x < VirusCount:
      ...
      virus_moved = (r == 0 and virus.alive == 1)
      if virus_moved:
          MoveVirus(virus, DinnerPlate)
      ...
      if virus_moved or VirusCount == 1:
          # Replication logic
          if (fast_rng.randint(0, virus.replicationrate) == 1 or
              ...
              if getattr(NewVirus, 'strain_id', 0) > 0:
          ...
          if fast_rng.randint(0, virus.chanceofdying) == 1:
              ...
              VirusCount -= 1
              continue
          elif NameCount >= VirusNameSpeedupCount:
      ...
      for v in range(0,DinnerPlate.height):
        for h in range(0,DinnerPlate.width):
          if (DinnerPlate.Playfield[v][h].alive == 0 and DinnerPlate.Playfield[v][h].name != 'EmptyObject'):
            DinnerPlate.set_playfield_empty(v, h)
    ...
    frame_sleep = (1.0 / TARGET_FPS) - frame_elapsed
    if frame_sleep > 0:
    ...
    if(VirusCount == 0):
      ...
      reset_strain_ids()
      DinnerPlate = CreateDinnerPlate(width=80, height=44, virus_count=VIRUS_START_COUNT)
      VirusCount = len(DinnerPlate.Viruses)
...
def LaunchOutbreak2(Duration = 10000, ShowIntro = True, StopEvent=None):
  ...
  if(ShowIntro == True):
    ...
    LED.ShowTitleScreen(
        ...
        LittleText          = 'INFECTION II',
        LittleTextRGB       = LED.MedGreen,
        LittleTextShadowRGB = (0,10,0), 
        ScrollText          = 'THE PLAGUE MUTATES AGAIN',
  ...
  PlayOutbreak2(Duration,StopEvent)
...
if __name__ == "__main__" :
  while(1==1):
    ...
    LED.SaveConfigData()
    print("After SAVE OutbreakGamesPlayed:",LED.OutbreakGamesPlayed)
    LaunchOutbreak2(100000,ShowIntro = False, StopEvent=None)        
